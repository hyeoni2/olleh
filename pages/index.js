...
export default function App() {
        ...
        data.forEach(sig => {
          if (!priceLinesRef.current[sig.id] && seriesRef.current) {
            ...
            priceLinesRef.current[sig.id] = seriesRef.current.createPriceLine({
              price: p, color: '#2962ff', title: `[${sig.user_name}]`, lineWidth: 2, lineStyle: 2
            });
          }
        });
      }
    };
    const loop = setInterval(fetchSignals, 10000); // 💡 깃허브 반영 시간을 위해 10초 주기
    fetchSignals();
  ...
  return (
    <div style={{ padding: '20px', backgroundColor: '#0b0e11', minHeight: '100vh', color: '#eaecef' }}>
